...
export default function App() {
  ...
  const handleTriggerInterstitial = async (actionName: string, onComplete: () => void) => {
    ...
    // ----------------------------------------
    if (mediationConfig.useRealAdMob && Capacitor.isNativePlatform()) {
      try {
        await AdMob.prepareInterstitial({ adId: 'ca-app-pub-8149561898063986/7523671298' }); // Interstitial Real do AdMob
        setIsInterstitialLoading(false);
        await AdMob.showInterstitial();
        onComplete();
      } catch (err) {
        console.error('Error with real AdMob Interstitial:', err);
        setIsInterstitialLoading(false);
        onComplete();
      }
      return;
  ...
  const handleConfirmSecureDelete = async (e: React.FormEvent) => {
    ...
    if (correctPassword && secureDeletePassword === correctPassword) {
      isPasswordCorrect = true;
    } else if (session?.password && secureDeletePassword === session.password) {
      isPasswordCorrect = true;
    } else if (supabase && session?.email) {
      try {
        setSecureDeleteError('Validando senha...');
        const { error: authError } = await supabase.auth.signInWithPassword({
          email: session.email,
          password: secureDeletePassword,
        });
        if (!authError) {
          isPasswordCorrect = true;
          setSecureDeleteError('');
          
          // Guardar na sessão e nos usuários registrados locais para validar deleções futuras offline
          const updatedSession = { ...session, password: secureDeletePassword };
          ...
          if (userIndex > -1) {
            registeredUsers[userIndex].password = secureDeletePassword;
          } else {
            registeredUsers.push({
              ...
              phone: session.phone,
              allowWhatsApp: session.allowWhatsApp,
            });
          }
          localStorage.setItem('ctrlpet_registered_users', JSON.stringify(registeredUsers));
        } else {
          setSecureDeleteError('Senha incorreta! Verifique os dados digitados.');
          return;
        }
      } catch (err) {
        console.error('Erro de rede ao validar senha:', err);
        setSecureDeleteError('Erro de conexão ao validar senha.');
        return;
      }
    }
  ...
  const handleAddCoOwner = async (e: React.FormEvent) => {
    ...
    // Choose the base public web URL for sharing. Fallback to production web URL if native or localhost.
    const publicUrl = (import.meta.env.VITE_PUBLIC_URL || 'https://ais-pre-vtbpym5r66yx6b2b5umtg3-717448426054.us-east1.run.app').replace(/\/$/, '');
    const baseOrigin = (isNative || isLocalhost) ? publicUrl : window.location.origin;
    
    // Ensure we don't carry '/index.html' or similar internal asset paths into the public web invitation link
    ...
    const link = `${baseOrigin}${cleanPath}?invite=${selectedPetId || ''}&by=${encodeURIComponent(session?.email || '')}&petName=${encodeURIComponent(petName)}`;
  ...
  const handleConfirmAccountDelete = async (e: React.FormEvent) => {
    ...
    if (correctPassword && deletePassword === correctPassword) {
      isPasswordCorrect = true;
    } else if (session?.password && deletePassword === session.password) {
      isPasswordCorrect = true;
    } else if (supabase && session?.email) {
      try {
        setDeleteError('Validando senha...');
        const { error: authError } = await supabase.auth.signInWithPassword({
          email: session.email,
        ...
        console.error('Erro de rede ao validar senha:', err);
        setDeleteError('Erro de conexão ao validar senha.');
        return;
      }
    }
