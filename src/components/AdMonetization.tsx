...
export interface MediationConfig {
  ...
  useRealAdMob: boolean; // Use real AdMob SDK instead of simulation
  activeNetworks: {
    AdMob: boolean;
    AdManager: boolean;
    AppLovin: boolean;
...
export const DEFAULT_MEDIATION_CONFIG: MediationConfig = {
  ...
  enableDirectSponsors: false, // <-- Desativado para testar AdMob e leilões
  useRealAdMob: false, // Habilitado simulador web
  activeNetworks: {
    AdMob: true,
    AdManager: true,
    AppLovin: true,
...
export function NativeAdCard({ config, categoryFilter = 'all' }: NativeAdCardProps) {
  ...
  useEffect(() => {
    // ----------------------------------------
    // REAL ADMOB NATIVE PLUGIN INTEGRATION
    // ----------------------------------------
    if (config.useRealAdMob && Capacitor.isNativePlatform()) {
      let isBannerActive = true;
      AdMob.showBanner({
        adId: 'ca-app-pub-8149561898063986/5656055855', // Banner 1 (Native Ad Card)
        adSize: BannerAdSize.BANNER,
        position: BannerAdPosition.TOP_CENTER,
        margin: 60, // Margem para não sobrepor o header
      }).catch(err => console.error('Error showing AdMob Banner 1:', err));

      setLoading(false);
      setAuctionResult({
        campaign: { ...SIMULATED_AD_CAMPAIGNS[0], network: 'AdMob', name: 'Banner 1 AdMob' },
        ecpm: 0,
        logs: []
      });
      ...
      return () => {
        ...
        // Importante: No capacitor geralmente só um banner fica ativo por vez. 
        // Ocultar ao desmontar para evitar bugs visuais.
...
export function AdaptiveBanner({ config }: AdaptiveBannerProps) {
  ...
  useEffect(() => {
    ...
    if (config.useRealAdMob && Capacitor.isNativePlatform()) {
      let isBannerActive = true;
      AdMob.showBanner({
        adId: 'ca-app-pub-8149561898063986/6067119396', // Banner Real do AdMob
        adSize: BannerAdSize.ADAPTIVE_BANNER,
        position: BannerAdPosition.BOTTOM_CENTER,
        margin: 0,
      }).catch(err => console.error('Error showing AdMob Banner:', err));
    ...
    // ----------------------------------------
    // FALLBACK: AD AUCTION SIMULATOR FOR WEB
    // ----------------------------------------
...
export function ContextualAdSlot({ config }: ContextualAdSlotProps) {
  ...
  useEffect(() => {
    if (config.useRealAdMob && Capacitor.isNativePlatform()) {
      let isBannerActive = true;
      AdMob.showBanner({
        adId: 'ca-app-pub-8149561898063986/5624266268', // Banner 3 (Contextual Ad Slot)
        adSize: BannerAdSize.BANNER,
        position: BannerAdPosition.TOP_CENTER,
        margin: 120, 
      }).catch(err => console.error('Error showing AdMob Banner 3:', err));
    ...
    // SIMULAÇÃO WEB: Executa o leilão buscando campanhas ativas
    setLoading(true);
    const timer = setTimeout(() => {
      // Tenta buscar um anúncio focado em "food" ou geral
      const res = runBiddingMediationAuction('ContextualAdSlot', config, 'food');
      const finalRes = res.campaign ? res : runBiddingMediationAuction('ContextualAdSlot', config);
      
      setAuctionResult(finalRes);
      
      addMediationLog({
        slot: 'ContextualAdSlot',
        action: finalRes.campaign ? `Preencheu com anúncio contextual` : 'Falha de Preenchimento (Fallback House Ad)',
        winnerNetwork: finalRes.campaign ? finalRes.campaign.network : 'House Ad',
        winnerEcpm: finalRes.campaign ? finalRes.ecpm : 0,
        status: finalRes.campaign ? 'filled' : 'no-fill'
      });
      setLoading(false);
    }, 450);
  ...
  if (loading) {
    return (
      <div className="w-full h-full min-h-[300px] box-border mx-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm animate-pulse flex flex-col justify-between">
        <div className="w-full h-32 bg-slate-200 dark:bg-slate-800 rounded-xl mb-4"></div>
        <div className="space-y-2 mb-4">
          <div className="h-4 bg-slate-200 dark:bg-slate-800 rounded w-3/4"></div>
          <div className="h-3 bg-slate-200 dark:bg-slate-800 rounded w-full"></div>
          <div className="h-3 bg-slate-200 dark:bg-slate-800 rounded w-5/6"></div>
        </div>
        <div className="h-10 bg-slate-200 dark:bg-slate-800 rounded-xl w-full mt-auto"></div>
      </div>
    );
  }
  ...
  if (config.useRealAdMob && Capacitor.isNativePlatform()) {
    return (
      <div className="w-full max-w-full box-border mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-sm text-center">
        <p className="text-xs text-slate-500">
          Banner 3 está sendo renderizado como uma View Nativa (Flutuante) pelo Capacitor.
        </p>
      </div>
    );
  }

  const campaign = auctionResult?.campaign;

  // Se o leilão venceu, exibe o Ad Card Contextual!
  if (campaign) {
    return (
      <div className="w-full max-w-full box-border mx-auto bg-white dark:bg-slate-900 border border-indigo-100 dark:border-indigo-950/40 rounded-2xl p-0 shadow-sm overflow-hidden group flex flex-col h-full relative transition-all hover:shadow-md">
        <div className="absolute top-3 left-3 bg-indigo-600 text-white text-[9px] font-black uppercase px-2 py-0.5 rounded shadow-sm z-10 tracking-wider">
          Ad ({campaign.network})
        </div>
        <div className="flex-1 min-h-[160px] w-full overflow-hidden relative border-b border-slate-100 dark:border-slate-850">
          <img 
            src={campaign.imageUrl} 
            alt={campaign.title} 
            referrerPolicy="no-referrer"
        ...
        <div className="p-5 flex flex-col justify-end">
          <div className="space-y-2">
            <h3 className="text-sm font-black text-slate-850 dark:text-slate-100 leading-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
              {campaign.title}
            </h3>
          ...
          <a 
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="mt-4 w-full py-2.5 bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/40 dark:hover:bg-indigo-950/80 text-indigo-600 dark:text-indigo-400 text-[11px] font-extrabold rounded-xl flex items-center justify-center gap-1.5 transition-colors border border-indigo-200/20 cursor-pointer"
          >
            <span>{campaign.ctaText}</span>
  ...
  // FALLBACK: House Ad do Clube de Benefícios (Se nenhum anúncio ganhar)
  return (
    <>
      <div className="bg-gradient-to-br from-[#1E1B4B] to-[#0F172A] border border-indigo-900/40 rounded-2xl p-6 shadow-xl text-white relative overflow-hidden group flex flex-col h-full">
        ...
        <div className="relative space-y-4 flex-1 flex flex-col">
          ...
          <div className="pt-2 flex justify-end mt-auto">
            <button
              ...
              className="px-5 py-2 w-full bg-gradient-to-r from-indigo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white text-xs font-black rounded-xl shadow-md flex items-center justify-center gap-1.5 cursor-pointer border border-indigo-400/20 transition-all"
            >
          ...
          <div className="text-[9.5px] text-indigo-300/60 font-semibold flex items-center justify-between select-none pt-2 border-t border-indigo-950">
            ...
            <span className="flex items-center gap-1 text-[11px] text-indigo-305">
              Seguro <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="mt-5 pt-5 border-t border-slate-800 space-y-6 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              {/* Real AdMob Toggle */}
              <div className="flex items-center justify-between p-3 bg-slate-850 rounded-xl border border-slate-800/60 mt-3">
                <div className="space-y-0.5">
                  <label className="text-[11px] font-bold text-slate-100 flex items-center gap-1.5">
                    Habilitar Real AdMob SDK
                  </label>
                  <p className="text-[10px] text-slate-400">
                    Oculta a interface Web e injeta o plugin Nativo
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => onChangeConfig({ ...config, useRealAdMob: !config.useRealAdMob })}
                  className={`w-11 h-6 rounded-full flex items-center px-1 transition-colors duration-300 ${config.useRealAdMob ? 'bg-indigo-600' : 'bg-slate-700'}`}
                >
                  <span className={`w-4 h-4 bg-white rounded-full shadow-md transform transition-transform duration-300 ${config.useRealAdMob ? 'translate-x-5' : 'translate-x-0'}`} />
                </button>
              </div>
            </div>
