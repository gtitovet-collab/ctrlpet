...
export default function ShareExportModule({
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const marginMm = 12; // Margem padrão (similar ao padding)
      const contentWidth = pdfWidth - (marginMm * 2);
      
      let currentY = marginMm;
      
      // Para evitar cortar texto ao meio, capturamos cada seção individualmente
      // O renderDossierContent retorna uma div wrapper (children[0]) que contém os itens
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        const canvas = await html2canvas(section, {
          scale: 2.5, // DPI aprimorado para nitidez
          useCORS: true,
          logging: false,
          backgroundColor: '#ffffff',
          windowWidth: 850,
          onclone: (clonedDoc) => {
            // Garante que o modo escuro do app esteja desativado no clone
        ...
        const childHeightMm = (canvas.height * contentWidth) / canvas.width;
        
        // Se a seção ultrapassar a altura restante e não for o início da página, cria nova página
        if (currentY + childHeightMm > pdfHeight - marginMm && currentY > marginMm) {
          pdf.addPage();
          currentY = marginMm;
        }
        
        pdf.addImage(imgData, 'JPEG', marginMm, currentY, contentWidth, childHeightMm);
        currentY += childHeightMm + 4; // gap de 4mm simulando o space-y-8
      }
