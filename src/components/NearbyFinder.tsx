...
// Fixed coordinates for fallback hub (Avenida Paulista, São Paulo but centered around the active GPS coordinate set)
const DEFAULT_CENTER = { lat: -22.5816, lng: -48.8580 };
...
export default function NearbyFinder() {
  ...
  // Detect and obtain user's location for full localization
  useEffect(() => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.warn("Permissão de localização negada ou indisponível. Usando São Paulo:", error);
          setLocationLoaded(true);
        }
      );
    } else {
  ...
  // Initialize places around user location dynamically to guarantee local relevance
  useEffect(() => {
    if (!locationLoaded) return;

    // Load from localStorage or seed initial data
    const stored = localStorage.getItem('ctrlpet_nearby_places');
    if (stored) {
      setPlaces(JSON.parse(stored));
    } else {
      const seeded = generateDefaultLocalPlaces(userLocation);
      setPlaces(seeded);
      localStorage.setItem('ctrlpet_nearby_places', JSON.stringify(seeded));
    }
  ...
  const savePlaces = (updatedList: Place[]) => {
    ...
    localStorage.setItem('ctrlpet_nearby_places', JSON.stringify(updatedList));
  };
            ...
            <div className="w-full h-full bg-slate-50 dark:bg-[#070b13] relative flex flex-col items-center justify-center p-6 border-b border-slate-200 dark:border-slate-850 overflow-hidden select-none">
              ...
              <div className="absolute top-4 right-4 z-10 bg-white/95 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 p-3 rounded-xl max-w-xs shadow-md space-y-1.5 text-left text-xs font-sans animate-in fade-in">
                ...
                <p className="text-[10px] text-slate-500 leading-snug">
                  Rodando no <strong>Modo Simulação Geolocalizada</strong>. Exibindo {places.length} locais da sua região.
                </p>
                <div className="border-t border-slate-100 dark:border-slate-800 pt-1.5 flex items-center justify-between text-[9px] text-[#4040F2] font-semibold">
                  <span>GPS ATIVO (-22.5816, -48.8580)</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="absolute bottom-4 left-4 right-4 z-10 bg-indigo-950/95 border border-indigo-700/60 p-3 px-4 rounded-xl text-white text-xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 shadow-xl leading-normal no-print">
            <div className="space-y-0.5">
              ...
              <p className="text-[10px] text-indigo-200">
                {hasValidKey 
                  ? "Mapa dinâmico real (@googlemaps/js-api-loader) ativo focado nas coordenadas GPS (-22.5816, -48.8580)." 
