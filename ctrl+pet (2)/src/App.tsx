        ...
        <div className="flex-1 overflow-y-auto content-start bg-[#F8FAFC] dark:bg-slate-950">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm grid grid-cols-1 lg:grid-cols-3 gap-6 relative print:hidden" id="sharing-danger-controls">
            ...
            <div className="space-y-4">
              ...
              {isGuardaTableMissing && (
                <div className="mt-3 p-3 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900/30 rounded-lg space-y-2 text-left">
                  <p className="text-[11px] font-bold text-amber-800 dark:text-amber-400 flex items-center gap-1">
                    ⚠️ Tabela 'GuardaCompartilhada' não encontrada no Supabase
                  </p>
                  <p className="text-[10px] text-slate-600 dark:text-slate-400 leading-relaxed">
                    Sua tabela de guarda compartilhada na nuvem não foi criada ou está desatualizada no Supabase. Para habilitar sincronização em tempo real entre diferentes celulares, execute o script SQL abaixo no <strong>SQL Editor</strong> do Supabase:
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                      const sql = `-- Copie e execute este script no SQL Editor do Supabase:\n\nCREATE TABLE IF NOT EXISTS public."GuardaCompartilhada" (\n    id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,\n    pet_id text NOT NULL REFERENCES public."Pets"(id) ON DELETE CASCADE,\n    co_owner_email text NOT NULL,\n    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,\n    CONSTRAINT unique_pet_co_owner UNIQUE (pet_id, co_owner_email)\n);\n\nALTER TABLE public."GuardaCompartilhada" ENABLE ROW LEVEL SECURITY;\n\nCREATE POLICY "Permitir acesso individual por e-mail de co-proprietário" ON public."GuardaCompartilhada"\n    AS PERMISSIVE FOR ALL TO public\n    USING (co_owner_email = auth.jwt() ->> 'email' OR EXISTS (\n        SELECT 1 FROM public."Pets" WHERE id = pet_id AND user_id = auth.jwt() ->> 'email'\n    ))\n    WITH CHECK (co_owner_email = auth.jwt() ->> 'email' OR EXISTS (\n        SELECT 1 FROM public."Pets" WHERE id = pet_id AND user_id = auth.jwt() ->> 'email'\n    ));\n\nCREATE INDEX IF NOT EXISTS idx_guarda_pet_id ON public."GuardaCompartilhada"(pet_id);\nCREATE INDEX IF NOT EXISTS idx_guarda_email ON public."GuardaCompartilhada"(co_owner_email);`;
                      navigator.clipboard.writeText(sql);
                      setToastNotification('Script SQL copiado com sucesso! cole no SQL Editor do Supabase.');
                    }}
                    className="w-full py-1.5 px-2 bg-amber-600 hover:bg-amber-700 text-white font-bold text-[10px] rounded-lg transition-colors cursor-pointer text-center"
                  >
